...
from app.events.schemas import SEvents, EventTypeEnum
...
@router.get("")
@cache(expire=60)
async def get_events(event_type: EventTypeEnum = None) -> list[SEvents]:
    if event_type:
...
@router.get("/{location}")
@cache(expire=60)
async def get_events_by_location(location: str, event_type: EventTypeEnum = None) -> list[SEvents]:
